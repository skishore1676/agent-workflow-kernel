...
class BlackboardPointerPacket:
    ...
    def from_mapping(cls, data: Mapping[str, Any]) -> "BlackboardPointerPacket":
        ...
        return cls(
            ...
            prompt_hash=_optional_text(data, "prompt_hash"),
            prompt_context_ref=_optional_text(data, "prompt_context_ref"),
            prompt_refs=_prompt_refs(data.get("prompt_refs")),
    ...
    def _record(self, pointer: BlackboardPointerPacket) -> dict[str, Any]:
        ...
        return {
            ...
            "awk": {
                ...
                "prompt_hash": pointer.prompt_hash,
                "prompt_context_ref": pointer.prompt_context_ref,
                "prompt_refs": list(pointer.prompt_refs),
...
def _prompt_refs(value: Any) -> tuple[dict[str, Any], ...]:
    if not isinstance(value, (list, tuple)):
        return ()
    refs: list[dict[str, Any]] = []
    for item in value:
        if isinstance(item, Mapping):
            refs.append({str(key): item[key] for key in sorted(item)})
    return tuple(refs)
